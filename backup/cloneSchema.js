...
async function waitUntilAttributesAvailable(colId) {
    console.log(`  Waiting for attributes in collection "${colId}" to process...`);
    while (true) {
        const list = await databasesDev.listAttributes(dbId, colId);
        const processing = list.attributes.filter((a) => a.status !== 'available' && a.status !== 'failed');
...
async function cloneSchema() {
    ...
    for (const col of collections) {
        ...
        try {
            ...
            await databasesDev.createCollection(dbId, colId, col.name, col.$permissions || col.permissions, col.documentSecurity);
        }
    }
    ...
    for (const col of collections) {
        ...
        const attributes = attrList.attributes;
    ...
    for (const col of collections) {
        ...
        for (const attr of attributes) {
            ...
            await databasesDev.createRelationshipAttribute(dbId, colId, attr.relatedCollection, attr.relationType, attr.twoWay, key, attr.twoWayKey, attr.onDelete);
            await sleep(1000);
        }
    }
    ...
    for (const col of collections) {
        ...
        for (const idx of indexes) {
            ...
            await databasesDev.createIndex(dbId, colId, key, idx.type, idx.attributes, idx.orders);
            await sleep(1000);
        }
    }
    ...
    try {
        ...
        for (const b of buckets) {
            try {
                ...
                await storageDev.createBucket(b.$id, b.name, b.$permissions || b.permissions, b.fileSecurity, b.enabled, b.maximumFileSize, b.allowedFileExtensions, b.compression, b.encryption, b.antipassback);
            }
        }
    }
    catch (err) {
