import { Client as AppwriteClient, Databases, Users, Storage, InputFile, Teams, Permission, Role } from 'node-appwrite';
...
function getDocumentPermissions(orgId) {
    if (!orgId)
        return [];
...
async function startRestoration() {
    ...
    try {
        if (driveFolderId) {
            ...
            const files = (listRes.data.files || []).filter((f) => f.name && f.name.startsWith('ttt_snapshot_'));
            if (files.length === 0) {
            ...
            if (targetTimestamp) {
                const found = files.find((f) => f.name?.includes(targetTimestamp));
                if (found) {
                    targetFile = found;
                }
                else {
                ...
                for (const archName of archives) {
                    try {
                        ...
                        if (archSearch.data.files && archSearch.data.files.length > 0) {
                            ...
                            fs.writeFileSync(localArchPath, Buffer.from(archDl.data));
        ...
        for (const u of usersList) {
            try {
                ...
                if (existingUser) {
                    ...
                    await usersService.create(u.$id, u.email, u.phone || undefined, tempPassword, u.name || undefined);
                    await usersService.updateStatus(u.$id, u.status);
            ...
            try {
                ...
                for (const member of memberships) {
                    try {
                        ...
                        const exists = currentMembers.memberships.some((m) => m.userId === member.userId);
                        if (!exists) {
                            console.log(`Adding member ${member.userName} (${member.userId}) to team ${team.name}`);
                            await teamsService.createMembership(team.$id, member.roles, member.userEmail || undefined, member.userId || undefined, member.userPhone || undefined, process.env.VITE_APP_URL || 'https://lorryguru.in', member.userName || undefined);
                        }
                    }
                    catch (memErr) {
        ...
        for (let i = 0; i < fileManifest.length; i += chunkSize) {
            ...
            await Promise.all(chunk.map(async (fileItem) => {
                ...
                try {
                    ...
                    if (fileBuffer) {
                        try {
                            await storageService.createFile(bucketId, fileId, InputFile.fromBuffer(fileBuffer, filename));
