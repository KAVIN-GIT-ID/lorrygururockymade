...
export async function runCompleteBackup() {
    ...
    try {
            ...
            try {
                ...
                for (const file of files) {
                    try {
                        ...
                        const alreadyBackedUp = manifest.files.some((f) => f.key === fileKey);
                        if (alreadyBackedUp) {
                            continue;
                        }
        ...
        fs.writeFileSync(path.join(localBackupDir, 'files_manifest.json'), JSON.stringify(exportedFiles, null, 2), 'utf8');
        ...
        if (driveFolderId && drive) {
            try {
                ...
                const driveSnapshots = (driveSnapshotsRes.data.files || []).filter((f) => f.name && f.name.startsWith('ttt_snapshot_'));
                // Sort explicitly by YYYYMMDD_HHMMSS pattern in name just in case API timestamps vary
                driveSnapshots.sort((a, b) => (b.name || '').localeCompare(a.name || ''));
