...
export default function MobileOutstandingView({
  ...
  return (
    <div class="space-y-4 font-sans pb-8">
      {/* Top Banner Overview */}
      <div class="bg-gradient-to-tr from-amber-600 to-orange-700 text-white rounded-3xl p-5 shadow-md flex items-center justify-between">
        <div class="space-y-1">
          <span class="text-[10px] uppercase font-bold tracking-widest text-orange-200">Total Outstanding Balance</span>
          <h2 class="text-2xl font-black font-mono">₹{totalOutstandingSum.toLocaleString('en-IN')}</h2>
        </div>
      ...
      {/* Tabs */}
      <div class="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-2xl border border-slate-200/60 dark:border-slate-800">
        <button
          onClick={() => {
          ...
          class={`flex-1 py-2.5 text-xs font-bold rounded-xl transition duration-150 cursor-pointer text-center ${
      ...
      <div class="space-y-3">
        {outstandingTab() === 'office' ? (
          topOutstandingOffices.length === 0 ? (
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-8 text-center">
              <Check class="w-8 h-8 text-emerald-500 mx-auto mb-2" />
              <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">All office balances are fully settled.</p>
            </div>
          ) : (
            topOutstandingOffices.map(({ officeName, amount }) => {
              ...
              return (
                <div  class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl overflow-hidden shadow-2xs">
                  <div 
                    onClick={() => {
                      setExpandedItem(isExpanded ? null : officeName);
                      setSwipedId(null);
                    }}
                    class="p-4 flex items-center justify-between gap-4 cursor-pointer hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition"
                  >
                    <div class="flex items-center gap-2.5 min-w-0">
                      <div class="w-8 h-8 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-455 flex items-center justify-center border border-rose-500/15 shrink-0">
                        <Building2 class="w-4 h-4" />
                      </div>
                      <span class="text-xs font-black text-slate-850 dark:text-slate-200 uppercase truncate font-mono tracking-wider">{officeName}</span>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="text-xs font-mono font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 border border-red-150 dark:border-red-900/40 px-2 py-0.5 rounded shadow-3xs">
                        ₹{amount.toLocaleString('en-IN')}
                      </span>
                      {isExpanded ? <ChevronUp class="w-4 h-4 text-slate-400" /> : <ChevronDown class="w-4 h-4 text-slate-400" />}
                    </div>
                  </div>

                  {isExpanded && (
                    <div class="border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/40 dark:bg-slate-950/20 p-4 space-y-3 font-sans">
                      <div class="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-2">
                        <span class="font-semibold">Trucks Used: {details.truckList}</span>
                        <div class="flex gap-2 shrink-0">
                          <button
                            ...
                            class="flex items-center gap-1 px-2.5 py-1 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-[9px] font-bold border border-slate-200 dark:border-slate-700/60 transition cursor-pointer"
                          >
                          ...
                          <button
                            ...
                            class="flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 hover:bg-blue-100 dark:bg-blue-900/30 dark:hover:bg-blue-900/50 text-blue-600 dark:text-blue-400 rounded-lg text-[9px] font-extrabold transition cursor-pointer border border-blue-100 dark:border-blue-800/85"
                          >
                      ...
                      <div class="space-y-2.5 max-h-[260px] overflow-y-auto pr-1">
                        <p class="text-[8px] text-slate-400 italic mb-1">Swipe left on any consignment to quickly Register Payment.</p>
                        {details.detailsList.map((seg: any, idx: number) => {
                          const cardId = `off-${officeName}-${seg.subTripId}`;
                          return (
                            <div 
                               
                              class="relative overflow-hidden rounded-2xl bg-slate-100 dark:bg-slate-800 shadow-3xs"
                              {...getTouchHandlers(cardId)}
                            >
                              {/* Background Pay Slide Action */}
                              <div class="absolute inset-0 flex justify-end bg-blue-650 z-0">
                                <button
                                  onClick={(e) => {
                                    e.stopPropagation();
                                  ...
                                  class="h-full w-20 bg-blue-600 text-white font-extrabold text-xs flex items-center justify-center cursor-pointer transition active:bg-blue-750"
                                >
                                  Pay
                                </button>
                              </div>
                              ...
                              <div 
                                ...
                                class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800/60 p-2.5 text-[10px] space-y-1 font-sans relative z-10 transition-transform duration-200"
                              >
                                <div class="flex justify-between items-center">
                                  <span class="font-extrabold text-slate-800 dark:text-slate-200">{seg.truckNo}</span>
                                  <span class="text-slate-400 font-medium flex items-center gap-1">
                                    <Calendar class="w-2.5 h-2.5" />
                                    {seg.date}
                                  </span>
                                </div>
                                <div class="text-slate-500 dark:text-slate-400 font-semibold truncate">
                                  Route: {seg.routeFrom || '—'} → {seg.routeTo || '—'}
                                </div>
                                <div class="flex justify-between items-center border-t border-slate-50 dark:border-slate-800/40 pt-1.5 mt-1">
                                  <div class="flex gap-3 text-slate-400 font-medium">
                                    <span>Billed: <strong class="text-slate-600 dark:text-slate-300">₹{seg.income.toLocaleString()}</strong></span>
                                    <span>Received: <strong class="text-slate-600 dark:text-slate-300">₹{seg.advance.toLocaleString()}</strong></span>
                                  </div>
                                  <span class="font-bold text-red-600 dark:text-red-400 font-mono">₹{seg.balance.toLocaleString()}</span>
                                </div>
                              </div>
                            </div>
                          );
          ...
          topOutstandingTrucks.length === 0 ? (
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800/80 rounded-3xl p-8 text-center">
              <Check class="w-8 h-8 text-emerald-500 mx-auto mb-2" />
              <p class="text-xs text-slate-500 dark:text-slate-400 font-medium">All truck balances are fully settled.</p>
            </div>
          ) : (
            topOutstandingTrucks.map(({ truckNo, amount }) => {
              ...
              return (
                <div  class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/80 rounded-3xl overflow-hidden shadow-2xs">
                  ...
                  >
                    <div class="flex items-center gap-2.5 min-w-0">
                      <div class="w-8 h-8 rounded-xl bg-blue-500/10 text-blue-600 dark:text-blue-400 flex items-center justify-center border border-blue-500/15 shrink-0">
                        <Navigation class="w-4 h-4 rotate-45" />
                      </div>
                      <span class="text-xs font-black text-slate-850 dark:text-slate-200 uppercase truncate font-mono tracking-wider">{truckNo}</span>
                    </div>
                    <div class="flex items-center gap-2">
                      <span class="text-xs font-mono font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/30 border border-red-150 dark:border-red-900/40 px-2 py-0.5 rounded shadow-3xs">
                        ₹{amount.toLocaleString('en-IN')}
                  ...
                  {isExpanded && (
                    <div class="border-t border-slate-100 dark:border-slate-800/80 bg-slate-50/40 dark:bg-slate-950/20 p-4 space-y-3 font-sans">
                      <div class="flex items-center justify-between text-[10px] text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-slate-800 pb-2">
                        <span class="font-semibold">Loading Offices: {details.officeList}</span>
                        <div class="flex gap-2 shrink-0">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                      ...
                      <div class="space-y-2.5 max-h-[260px] overflow-y-auto pr-1">
                        <p class="text-[8px] text-slate-400 italic mb-1">Swipe left on any office segment to quickly Register Payment.</p>
                        {details.detailsList.map((seg: any, idx: number) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                <div class="flex justify-between items-center">
                                  <span class="font-extrabold text-slate-800 dark:text-slate-200">{seg.office}</span>
                                  <span class="text-slate-400 font-medium flex items-center gap-1">
                                    <Calendar class="w-2.5 h-2.5" />
                                    {seg.date}
                                  </span>
      ...
      {quickPayTarget() && (
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-[100] p-4 no-print animate-fade-in">
          <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 p-6 space-y-5 shadow-2xl max-w-sm w-full animate-scale-up font-sans text-left">
            <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-3">
              <div>
                <h3 class="text-sm font-bold text-slate-800 dark:text-white uppercase tracking-wider font-sans flex items-center gap-1.5">
                  <CreditCard class="w-4 h-4 text-blue-600" />
                  Register Payment
                </h3>
                <p class="text-[10px] text-slate-500 mt-0.5 font-medium">
                  Quick recording: <strong class="text-blue-600">{quickPayTarget().route}</strong>
                </p>
              </div>
              <button
                type="button"
                onClick={() => setQuickPayTarget(null)}
                class="text-slate-400 hover:text-slate-600 p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition cursor-pointer"
              >
                <X class="w-4 h-4" />
              </button>
            </div>

            <div class="grid grid-cols-2 gap-4 text-xs font-semibold text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-slate-200/50 dark:border-slate-800">
              <div>
                <span class="text-[9px] text-slate-400 uppercase font-bold block">Office Branch</span>
                <span class="text-slate-850 dark:text-white font-bold truncate block">{quickPayTarget().officeName}</span>
              </div>
              {quickPayTarget().truckNo && (
                <div>
                  <span class="text-[9px] text-slate-400 uppercase font-bold block">Truck No</span>
                  <span class="text-slate-850 dark:text-white font-mono font-bold tracking-wider block">{quickPayTarget().truckNo}</span>
                </div>
              )}
            </div>
            ...
            <div class="space-y-4 text-xs">
              <div>
                ...
                <input
                  ...
                  class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-250 dark:border-slate-800 text-slate-850 dark:text-white rounded-lg px-3 py-2 font-mono focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-slate-950 font-medium"
                />
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-250 dark:border-slate-800 text-slate-850 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-slate-950 font-bold"
                >
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="₹0.00"
                  class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-250 dark:border-slate-800 text-slate-850 dark:text-white rounded-lg px-3 py-2 font-mono font-bold text-right focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-slate-950 text-base text-emerald-700 dark:text-emerald-450"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-250 dark:border-slate-800 text-slate-805 dark:text-white rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500 focus:bg-white dark:focus:bg-slate-950 font-medium"
                />
              </div>
            </div>
            ...
            <div class="flex justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
              <button
                type="button"
                onClick={() => setQuickPayTarget(null)}
                class="px-4 py-2 text-xs font-bold text-slate-500 hover:text-slate-800 dark:text-slate-400 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg transition cursor-pointer"
              >
                Cancel
              </button>
              <button
      ...
      <div class="text-[10px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-900 p-3 rounded-2xl border border-slate-200/50 dark:border-slate-800 flex items-start gap-2 shadow-3xs">
