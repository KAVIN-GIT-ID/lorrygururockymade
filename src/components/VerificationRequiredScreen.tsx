...
export const VerificationRequiredScreen: Component<VerificationRequiredScreenProps> = ({
  currentUser,
  currentUserRights,
  userRightsList,
  setUserRightsList,
        ...
        <div class="space-y-5">
          ...
          <div class="bg-slate-50 dark:bg-slate-950/40 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80 space-y-3">
            <div class="flex justify-between items-center">
              ...
              <span class={`text-[10px] px-2 py-0.5 rounded-full font-bold ${currentUserRights.isEmailVerified
            ...
            <p class="text-[10px] text-slate-500 dark:text-slate-400">
              Registered Email: <span class="text-slate-800 dark:text-slate-200 font-mono font-medium">{currentUser.email}</span>
            </p>
            ...
            {!isAppwriteConfigured() && !currentUserRights.isEmailVerified && (
              <button
                ...
                class="w-full py-1.5 border border-dashed border-slate-200 dark:border-slate-800 hover:border-blue-500/50 text-slate-500 dark:text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 rounded-xl text-[10px] font-bold transition-all cursor-pointer"
              >
          ...
          <div class="bg-slate-50 dark:bg-slate-950/40 p-4 rounded-xl border border-slate-200 dark:border-slate-800/80 space-y-3">
            <div class="flex justify-between items-center">
              ...
              <span class={`text-[10px] px-2 py-0.5 rounded-full font-bold ${currentUserRights.isPhoneVerified
                  ? 'bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/30'
                  : 'bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800/30'
                }`}>
            ...
            <div class="flex justify-between items-center text-[10px] text-slate-500 dark:text-slate-400">
              <span>Mobile Number: <span class="text-slate-800 dark:text-slate-200 font-mono font-medium">{currentUserRights.phone || 'Not Set'}</span></span>
                  ...
                  <form onSubmit={async (e) => {
                    ...
                    <div class="relative">
                      <input
                        ...
                        class="w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 focus:border-blue-500 rounded-xl px-3 py-2 text-slate-800 dark:text-slate-200 text-xs focus:outline-none transition-all placeholder:text-slate-400"
                      />
                    </div>
                    <div class="flex gap-2">
                      <button
                        type="submit"
                        class="flex-1 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-[10px] transition-all cursor-pointer shadow-sm"
                      >
                        Verify Code
                      </button>
                      <button
                        ...
                        class="px-3 py-1.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-[10px] transition-all font-bold cursor-pointer"
                      >
                        Cancel
                      </button>
                    </div>
      ...
      {showPhoneUpdateModal && (
        <div class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 backdrop-blur-sm p-4 overflow-auto animate-fade-in">
          <div class="w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-6 space-y-4 text-left">
            ...
            <form onSubmit={handlePhoneUpdateSubmit} class="space-y-3">
              <div class="space-y-1">
