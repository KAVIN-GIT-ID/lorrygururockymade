...
export default function PayEmiModal({
  ...
  const [paymentDate, setPaymentDate] = useState(dueDateStr);
      ...
      <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-150 dark:border-slate-800">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-sm text-slate-900 dark:text-white font-sans uppercase tracking-wider">Record EMI Payment</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 text-slate-800 dark:text-slate-100 text-xs">
          <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-lg border border-slate-100 dark:border-slate-850 space-y-2">
            ...
            {loanType && (
              <div className="flex justify-between">
                <span className="text-slate-455">Loan Type:</span>
                <span className="font-semibold text-slate-900 dark:text-white">{loanType}</span>
              </div>
            )}
            <div className="flex justify-between">
              <span className="text-slate-455">Bank Name:</span>
              <span className="font-semibold text-slate-900 dark:text-white">{bankName}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-455">EMI Due Date:</span>
              <span className="font-semibold text-slate-900 dark:text-white">{dueDateStr}</span>
            </div>
            <div className="flex justify-between border-t border-slate-200 dark:border-slate-800 pt-2 mt-1">
              <span className="text-slate-455 font-bold">EMI Amount:</span>
              <span className="font-mono font-extrabold text-blue-600 dark:text-blue-400 text-sm">₹{emiAmount.toLocaleString('en-IN')}</span>
            </div>
          </div>
          ...
          {/* Payment Account */}
          <div className="space-y-1.5">
            <label className="block text-[10px] text-slate-400 font-bold uppercase tracking-wider">Payment Account</label>
            <div className="relative">
