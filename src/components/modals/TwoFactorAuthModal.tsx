...
interface Setup2FAModalProps {
  ...
  secret: string;
  onConfirmEnable: (secret: string, code: string, password?: string) => Promise<void>;
}
            ...
            <div className="bg-white p-2 rounded-lg">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=${encodeURIComponent(
                  `otpauth://totp/FleetTrack:${currentUserEmail}?secret=${secret}&issuer=FleetTrack`
                )}`}
          ...
          <div className="space-y-3">
            <div className="space-y-1.5">
              ...
              <input
                ...
                className="w-full bg-slate-950 border border-slate-800 focus:border-blue-500 rounded-lg px-3 py-2 text-slate-200 text-xs font-semibold focus:outline-none placeholder:text-slate-700 font-mono text-center tracking-widest"
              />
            </div>
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  await onConfirmEnable(secret, code, password);
                  onClose();
                } catch (err: any) {
...
interface Disable2FAModalProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirmDisable: (code: string, password?: string) => Promise<void>;
}
          ...
          <div className="flex gap-2 pt-2 border-t border-slate-800/60 mt-4 justify-end">
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  setIsSubmitting(true);
                  await onConfirmDisable(code, password);
                  onClose();
                } catch (err: any) {
                  setError(err.message || 'Verification or password invalid.');
                } finally {
              ...
              className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white text-xs font-bold rounded-lg transition-all shadow-md shadow-rose-600/10 cursor-pointer disabled:opacity-50"
            >
