...
import { Plus, Edit2, Trash2, User, Phone, FileText, CheckCircle, XCircle, Calculator, Coins, TrendingUp, Wallet, ArrowUpRight, ArrowDownLeft, Receipt } from 'lucide-react';
      ...
      {showAddForm && (
        <form id="driver-form" onSubmit={handleSubmit} className="mb-6 p-4 md:p-5 bg-slate-50 rounded-lg border border-slate-255/70 animate-fade-in">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
              <label className="block text-xs font-semibold text-slate-650 mb-1">Driver Name <span className="text-red-500">*</span></label>
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-650 mb-1">Driving License No.</label>
              <div className="relative">
                <span className="absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400">
          ...
          <div className="flex justify-end gap-2.5 mt-5 border-t border-slate-200/50 pt-4">
            <button
              ...
              className="px-4 py-1.5 border border-slate-200 text-slate-600 rounded-lg hover:bg-slate-100 text-xs font-medium cursor-pointer"
            >
              Cancel
            </button>
            <button
              ...
              type="submit"
              className="px-4 py-1.5 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 text-xs shadow-xs cursor-pointer"
            >
              ...
              drivers.map((driver) => (
                <tr key={driver.id} className="hover:bg-slate-50/50 transition">
                  ...
                  <td className="py-3.5 px-4 font-mono text-slate-600">
                    {canViewDrivers ? (driver.licenseNo || '—') : <span className="text-slate-400 italic text-[11px] font-mono">[Restricted]</span>}
                  </td>
                  <td className="py-3.5 px-4 font-medium">
                    {driver.status === 'Active' ? (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-green-50 border border-green-200 text-green-700 text-[10px] font-semibold">
                        <CheckCircle className="w-2.5 h-2.5" /> Active on Duty
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-slate-50 border border-slate-200 text-slate-500 text-[10px] font-semibold">
                  ...
                  <td className="py-3.5 px-4 text-right pr-6">
                    <div className="flex items-center justify-end gap-1.5">
                      <button
                        ...
                        className="p-1 px-2 hover:bg-slate-100 rounded text-slate-600 hover:text-slate-900 font-semibold cursor-pointer text-xs flex items-center gap-0.5 disabled:opacity-40 disabled:cursor-not-allowed"
                      >
      ...
      <div id="driver-settlement-module" className="mt-8 pt-6 border-t border-slate-100">
        ...
        {(() => {
          ...
          drvTrips.forEach(t => {
            ...
            // 3. SubTrip specific cargo level loading/unloading
            if (t.subTrips) {
              t.subTrips.forEach(st => {
                const segDate = st.loadingDate || t.startDate;
                if (st.loadingPaidByDriver !== false && st.loadingExpense && st.loadingExpense > 0) {
                  computedTripDriverCredits.push({
                    id: `seg-load-${st.id}`,
                    date: segDate,
                    type: 'Cargo Loading Expense',
                    amount: st.loadingExpense,
                    notes: `Loading expense for cargo loading at ${st.officeName}`,
                ...
                if (st.unloadingPaidByDriver !== false && st.unloadingExpense && st.unloadingExpense > 0) {
                  computedTripDriverCredits.push({
                    id: `seg-unload-${st.id}`,
                    date: segDate,
                    type: 'Cargo Unload Expense',
                    amount: st.unloadingExpense,
                    notes: `Unloading expense at destination`,
                    reference: `Trip: ${t.tripNo} (${st.routeFrom} ➔ ${st.routeTo})`,
                    fromMode: 'Driver Advance Hand cash',
                    isDirect: false,
                    badgeStyle: 'bg-teal-50 text-teal-800 border-teal-200'
                ...
                if (st.brokeragePaidByDriver !== false && st.brokerageExpense && st.brokerageExpense > 0) {
                  computedTripDriverCredits.push({
                    id: `seg-brokerage-${st.id}`,
                    date: segDate,
                    type: 'Cargo Brokerage Expense',
                    amount: st.brokerageExpense,
                    notes: `Brokerage expense paid by Driver`,
                    reference: `Trip: ${t.tripNo} (${st.routeFrom} ➔ ${st.routeTo})`,
                    fromMode: 'Driver Advance Hand cash',
                    isDirect: false,
                    badgeStyle: 'bg-teal-50 text-teal-800 border-teal-200'
          ...
          const netOutstanding = totalExpAmount - totalAdvAmount;
          ...
          const timeline = [...advItems, ...expItems, ...computedTripDriverCredits].sort((a, b) => b.date.localeCompare(a.date));
                      ...
                      <tbody className="divide-y divide-slate-100 font-medium">
                        {timeline.map((item, idx) => (
                          <tr key={idx} className="hover:bg-slate-50/50 text-slate-700">
                            ...
                            <td className={`p-2.5 text-right font-mono font-extrabold pr-4 text-xs ${item.type.startsWith('Trip Advance') ? 'text-emerald-600' : 'text-slate-800'}`}>
                              {item.type.startsWith('Trip Advance') ? '(-)' : '(+)'} ₹{item.amount.toLocaleString()}
                            </td>
                          </tr>
                        ))}
                      </tbody>
