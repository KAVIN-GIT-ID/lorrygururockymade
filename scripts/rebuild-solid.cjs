...
execSync('"C:\\Program Files\\Microsoft Visual Studio\\2022\\Professional\\Common7\\IDE\\CommonExtensions\\Microsoft\\TeamFoundation\\Team Explorer\\Git\\cmd\\git.exe" checkout src/', { stdio: 'inherit' });

console.log('2. Expanding shorthands in App.tsx...');
execSync('node scripts/expand-app-shorthands.cjs', { stdio: 'inherit' });

console.log('2.5. Expanding shorthands in TyreMaster.tsx...');
execSync('node scripts/expand-tyre-shorthands.cjs', { stdio: 'inherit' });

console.log('2.6. Expanding shorthands in UserAccessControl.tsx...');
execSync('node scripts/expand-user-access-control-shorthands.cjs', { stdio: 'inherit' });

console.log('2.7. Expanding shorthands in TruckMaster.tsx...');
execSync('node scripts/expand-truck-shorthands.cjs', { stdio: 'inherit' });
...
console.log('10.2. Wrapping Lucide title props in TruckMaster.tsx...');
execSync('node scripts/fix-lucide-title-props.cjs', { stdio: 'inherit' });

console.log('10.3. Renaming conflicting parameters/variables in TyreMaster/UserAccessControl...');
execSync('node scripts/rename-conflicting-variables.cjs', { stdio: 'inherit' });

console.log('10.5. Running automatic signal discovery and fixes...');
execSync('node scripts/fix-signals-automatically.cjs', { stdio: 'inherit' });
