...
async function run() {
  console.log('=== Single Data Field Appwrite Restorer ===');
  ...
  for (const colId of collections) {
    ...
    try {
      ...
      console.log(`  + Ensured 'data' attribute in "${colId}"`);
    } catch (e) {}
    ...
    for (let i = 0; i < 30; i++) {
      try {
        ...
        const dataAttr = (colMeta.attributes || []).find(a => a.key === 'data');
    ...
    const colMeta = await databases.getCollection(databaseId, colId);
    const availableKeys = new Set((colMeta.attributes || []).filter(a => a.status === 'available').map(a => a.key));
    ...
    for (const item of items) {
      ...
      // 1. Pack full object into 'data'
      if (availableKeys.has('data')) {
        payload['data'] = JSON.stringify(item);
      }

      // 2. Add any other active columns if present in item
      for (const [k, v] of Object.entries(item)) {
        if (availableKeys.has(k) && v !== null && v !== undefined && !k.startsWith('$')) {
          payload[k] = v;
        }
      }
