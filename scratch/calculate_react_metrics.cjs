...
// Replicate local React state of TripForm
function calculateReactMetrics(trip) {
  ...
  try {
    ...
    list.documents.forEach(doc => {
      if (doc.tripNo === 'TRIP-2026-0008') {
        console.log('\n=== TRIP-2026-0008 calculated with React metrics logic ===');
        console.log(calculateReactMetrics(doc));
      }
    });
  } catch (err) {
    console.error(err);
